...
class ControlPanelTestCase(BaseRedisTestCase):
    def test__info__success(self):
        ...
        self.assertGreater(len(control_panel.server_info), 0)
        self.assertGreater(len(control_panel.client_info), 0)
        self.assertGreater(len(control_panel.memory_info), 0)
        self.assertGreater(len(control_panel.cpu_info), 0)
